...
압축기를 ViT 직후가 아니라 **H-Reducer(vision-to-text) 뒤**에 두는 게 이 논문의 미묘하지만 중요한 선택임. 먼저 용어 정리: ViT는 시각 feature를 **시각 공간(visual space)**에 뱉음 — 픽셀·외형·국소 패턴을 인코딩한, 아직 "보기만 한" 표현임. H-Reducer는 여기에 conv로 가로 feature를 묶고 FC로 차원을 맞춰, 이 feature를 **LLM이 쓰는 텍스트 feature 공간에 정렬(align)**함. 정렬 후의 시각 토큰은 사실상 "문서의 어느 텍스트 조각을 인코딩한 텍스트류 토큰"으로 취급할 수 있게 됨.

핵심 논리는 이 정렬을 압축의 전제로 삼는 것임. **정렬이 끝난 토큰을 압축하는 건 곧 텍스트를 요약하는 것과 같다**는 주장임. 이 논문 전체가 기대는 NLP의 context compression(문단·문서를 소수 요약 벡터로 눌러도 의미가 보존된다는 결과들)이 성립하려면, 압축 대상이 이미 "의미를 담은 텍스트류 표현"이어야 함. 그래서 V2T 뒤에서 압축하면 cross-attention이 의미 보존형 요약처럼 동작함. 반대로 **ViT 직후(정렬 전)에 압축하면** 아직 텍스트 의미로 조직되지 않은 raw 시각 feature를 뭉개는 셈이라, 무엇을 남기고 버릴지의 의미적 기준이 없어 **visually-situated 텍스트 정보를 더 많이 잃음**. 비유하면, 페이지를 픽셀 상태로 욱여넣어 요약하느냐 vs 일단 "글로 읽어낸" 뒤 요약하느냐의 차이임.

ablation(r4 vs r3)이 이 가설을 뒷받침함. V2T 뒤 압축(r3)이 ViT 뒤 압축(r4)보다 세 데이터셋 모두에서 우위임 — DocVQA 76.1 vs 75.7, **WTQ 35.1 vs 33.3**, ChartQA 69.2 vs 68.7. 격차 자체는 크지 않지만 일관됨. 특히 표 QA인 **WTQ에서 격차가 가장 큼** — 표는 텍스트 밀도·구조 의존이 가장 높은 요소라, 텍스트 정렬의 이득이 거기서 가장 또렷이 드러남. 이 선택이 DocOwl2를 일반 시각 feature를 압축하는 Mini-Gemini류와 구분짓는 지점이기도 함 — 같은 "압축"이어도, **무엇을 압축하느냐(raw 시각 vs 텍스트 정렬 후)**가 문서에서는 결과를 가른다는 게 저자들의 입장임.

### 2.6 정보의 두 축: 콘텐츠 vs 레이아웃

DocOwl2의 설계를 한눈에 잡으려면, 문서 정보를 **직교하는 두 축**으로 나눠 보면 됨. "무슨 글자/내용이냐"와 "어디에 뭐가 있냐"는 서로 파생되는 관계가 아니라 **각각 따로 얻어 압축 단계에서 결합**됨.

- **콘텐츠 축:** 글리프(시각) → V2T → 의미. "무슨 글자/내용이냐"
- **레이아웃 축:** global 저해상도 이미지 → 압축기 query. "어디에 뭐가 있냐"
- **압축기가 둘을 합침:** 레이아웃 틀(global query) × 의미 세부(sub-image K/V) → group cross-attention → 324 토큰

여기서 주의할 점은 레이아웃이 "의미들을 조합해서 나오는 결과"가 아니라는 것임. global 저해상도 이미지가 페이지의 공간 배치를 **미리** 잡아 압축의 좌표 틀(query)로 쓰이고, 거기에 의미 있는 세부 토큰(sub-image, key/value)이 채워짐. 비유하면 **빈 좌표 격자(레이아웃)를 먼저 깔고, 각 칸에 읽어낸 내용(의미)을 적어넣어 한 장으로 요약**하는 구조임 — 격자가 글씨에서 유도되는 게 아니라, 격자가 먼저 있고 글씨가 거기 들어감.
